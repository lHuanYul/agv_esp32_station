...
static const char *TAG = "wifi tr-re";
 ...
 */
int core_tcp_send(const char *remote_ip, uint16_t remote_port, const char *data, size_t len) {
...
void wifi_transceive_setup(void) {
    ...
    if (ret != pdPASS) {
        ESP_LOGE(TAG, "xTaskCreate(tcp_receive_task) failed");
    }
}
